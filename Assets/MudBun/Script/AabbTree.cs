...
namespace MudBun
{
  ...
  public class AabbTree<T> where T : class
  {
    ...
    public AabbTree(float fatBoundsRadius = 0.0f)
    {
      m_nodes = new Node[16];
    ...
    public int Fill(ComputeBuffer buffer, float aabbExtentsOffset = 0.0f)
    {
      ...
      for (int i = 0; i < m_nodes.Length; ++i)
      {
        ...
        m_pods[i].UserDataIndex = m_nodes[i].UserDataIndex;
      }
    ...
    public void UpdateProxy(int proxy, Aabb bounds, int userDataIndex)
    {
      ...
      if (m_nodes[proxy].Bounds.Contains(bounds))
      {
        ...
        Vector3 extentDelta = VectorUtil.Abs(m_nodes[proxy].Bounds.Extents - extents);
        float fat4 = 4.0f * FatBoundsRadius;
        if (extentDelta.x < fat4 && extentDelta.y < fat4 && extentDelta.z < fat4)
          return;
      }
    ...
    {
      var copy = new AabbTree<T>(m_fatBoundsRadius);

      copy.m_nodes = new Node[m_nodes.Length];
      copy.m_pods = new NodePod[m_pods.Length];
      m_nodes.CopyTo(copy.m_nodes, 0);
      m_pods.CopyTo(copy.m_pods, 0);
