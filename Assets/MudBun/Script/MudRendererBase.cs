  ...
  public abstract class MudRendererBase : MonoBehaviour
  {
    ...
    public struct Const
    {
      public struct KernelIndex
      {
        ...
        public int SurfaceNetsFlatNormal;
        public int SurfaceNetsSmoothNormal;
    ...
    public static int[] VoxelTreeBranchingFactors => new int[] { 8, 8, 4 };
    public static int VoxelNodeDepth => VoxelTreeBranchingFactors.Length;
    public static int ChunkVoxelDensity => VoxelTreeBranchingFactors.Aggregate((x, y) => x * y);
    ...
    public static int[] NoiseCacheDimensionInts => new int[] { 256, 128, 256 };
    public static float[] NoiseCacheDimensionFloats => NoiseCacheDimensionInts.Select(x => (float)x).ToArray();
    public static readonly float NoiseCacheDensity = 32.0f;
    public static float[] NoiseCachePeriod => NoiseCacheDimensionInts.Select(x => x / NoiseCacheDensity).ToArray();
    ...
    private static ComputeBuffer s_edgeTableBuffer;
    ...
    public static int GlobalResourceGpuMemoryAllocated
    {
      get
      {
        ...
        if (s_edgeTableBuffer != null)
          bytes += s_edgeTableBuffer.stride * s_edgeTableBuffer.count;
    ...
    [Range(0.0f, 0.1f)] public float SmoothNormalBlurAbsolute = 0.0f;
    ...
    [Range(0.1f, 10.0f)] public float SplatJitterNoisiness = 1.0f;
    ...
    protected AabbTree<MudBrushBase> m_aabbTree = new AabbTree<MudBrushBase>(AabbTreeFatBoundsRadius);
    ...
    [SerializeField] [HideInInspector] public bool MeshGenerationLockOnStartByEditor = false;
    ...
    public Mesh GenerateMesh(GeneratedMeshType meshType, bool async, Mesh mesh = null)
    {
      Transform [] aBone;
      return GenerateMesh(meshType, null, out aBone, async, mesh);
    }

    public Mesh GenerateMesh(GeneratedMeshType meshType, Transform rootBone, out Transform [] aBone, bool async, Mesh mesh = null)
    {
      aBone = null;
    ...
    private void OnGenPointsBufferRead(AsyncGPUReadbackRequest request)
    {
      ...
      BuildMesh(pendingMeshData.Mesh, pendingMeshData.MeshType, aGenPoint, pendingMeshData.RootBone, pendingMeshData.Bones, pendingMeshData.DoRigging);
    ...
    private void BuildMesh(Mesh mesh, GeneratedMeshType meshType, NativeArray<GenPoint> aGenPoint, Transform rootBone, List<Transform> bones, bool doRigging)
    {
      ...
      if (meshType == GeneratedMeshType.Standard)
      {
        ...
        mesh.SetUVs(0, aEmissionHash);
        mesh.SetUVs(1, aMetallicSmoothness);
        if (aBoneWeight != null 
            && rootBone != null)
        {
    ...
    public virtual Mesh AddCollider(GameObject go, bool async, Mesh mesh = null, bool makeRigidBody = false) { return null; }
    public virtual Mesh AddLockedStandardMesh(GameObject go, bool autoRigging, bool async, Mesh mesh = null) { return null; }

    public virtual void LockMesh(bool autoRigging, bool async, Mesh mesh = null)
    {
      m_isMeshLocked = true;
    }
    ...
    protected virtual void OnEnable()
    {
      ...
      if (RenderMaterialMesh != null)
        ResourcesUtil.ValidateDefaultMaterial(ref RenderMaterialMesh);
      else
        RenderMaterialMesh = ResourcesUtil.DefaultMeshMaterial;

      if (RenderMaterialSplats != null)
        ResourcesUtil.ValidateDefaultMaterial(ref RenderMaterialSplats);
      else
        RenderMaterialSplats = ResourcesUtil.DefaultSplatMaterial;
    ...
    public static void ValidateShaderConstantId()
    {
      ...
      Const.EdgeTable = Shader.PropertyToID("edgeTable");
    ...
    private bool ValidateGlobalResources()
    {
          ...
          || s_computeMeshLock == null)
      {
        ...
        // marching cubes
        {
          ...
          ComputeManager.RegisterConstantId(s_computeMarchingCubes, Const.EdgeTable);
      ...
      if (s_edgeTableBuffer == null)
      {
        s_edgeTableBuffer = new ComputeBuffer(256, sizeof(int));
        s_edgeTableBuffer.SetData(MarchingCubes.EdgeTable);
        ComputeManager.SetBuffer(Const.EdgeTable, s_edgeTableBuffer);
        m_needsCompute = true;
      }
    ...
    private bool ValidateLocalResources()
    {
    ...
    protected void DisposeGlobalResources()
    {
      ...
      if (s_edgeTableBuffer != null)
      {
        s_edgeTableBuffer.Release();
        s_edgeTableBuffer = null;
      }
      ...
      switch (MeshingMode)
      {
        case MeshingModeEnum.SurfaceNets:
          ...
          */
          ...
          {
            ComputeManager.SetInt(Const.SurfaceNetsBinarySearchIterations, 10);
        ...
        case MeshingModeEnum.DualContouring:
          ...
          */
          ...
          {
            ComputeManager.SetInt(Const.DualContouringBinarySearchIterations, 12);
